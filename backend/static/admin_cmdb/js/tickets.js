    ...
    createApp({
        ...
        methods: {
            ...
            async fetchTechnicians() {
                try {
                    const res = await window.apiClient.get('/staff/auth/users/', { params: { role: 'technicien' } });
                ...
                try {
                    ...
                    if (error.response) {
                        if (error.response.data && error.response.data.error) {
                            ...
                            errorMessage = 'Données de transition invalides';
                        } else if (error.response.status === 403) {
                            errorMessage = 'Transition non autorisée';
                        } else {
        ...
        methods: {
            ...
            async transitionTicket(toStatus) {
                ...
                try {
                    ...
                    let errorMessage = 'Erreur lors de la transition';
                    if (error.response) {
                        if (error.response.data && error.response.data.error) {
                            errorMessage = error.response.data.error;
                        } else if (error.response.status === 400) {
