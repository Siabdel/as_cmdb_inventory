...
# -*- coding: utf-8 -*-
"""
test_print_v3.py - Bixolon SRP-350
...
class BixolonSRP350:
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    ...
                    print(f"⚠️ Impossible de détacher le kernel: {e}")
                
                # Claimer l'interface
                try:
                    usb.util.claim_interface(self.device, self.INTERFACE)
                except usb.core.USBError as e:
                    print(f"⚠️ Interface déjà claimée: {e}")
                ...
                if e.errno == 16:
                    print("💡 Solution: Blacklistez usblp et redémarrez")
                elif e.errno == 13:
                    print("💡 Solution: Vérifiez les règles udev")
    ...
    def print_cmdb_label(self, asset_id, qr_data=None, barcode_data=None):
        ...
        try:
            ...
            # QR Code (CORRIGÉ - variable complète)
            if qr_data is not None:
                self.printer.set(align='center')
                self.printer.qr(qr_data, size=8, ec='L')
                self.printer.text("\n")
            
            # Code-barres (CORRIGÉ - variable complète)
