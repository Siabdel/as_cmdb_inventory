...
# -*- coding: utf-8 -*-
"""
test_print_v4.py - Bixolon SRP-350
...
class BixolonSRP350:
    ...
    def connect(self, max_retries=3):
        """Connexion sans modifier la configuration USB"""
        for attempt in range(max_retries):
            try:
                ...
                # Vérifier la configuration active (test_usb.md : bConfigurationValue = 1)
                try:
                ...
                try:
                    ...
                    print(f"⚠️ Interface claim: {e}")
                
                # ⭐ CRUCIAL : Initialiser avec skip_config=True
                # Évite l'erreur "Could not set configuration: [Errno 16]"
                self.printer = Usb(
                    idVendor=self.VID,
                    idProduct=self.PID,
                    ...
                    out_ep=self.OUT_EP,
                    timeout=5000,
                    skip_config=True  # ⭐ NE PAS MODIFIER LA CONFIG USB
                )
